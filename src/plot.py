...
def main(experiment_dir, k, aug_factors, timings, raw_size):
    ...
    if download:
        for i in range(1, k+1):
            ...
            l += [('t', "%sfold_%i/raw_%i/runs/raw/results.csv"%(experiment_dir, 10+i, raw_size), 10+i)]
            for j in aug_factors:
                l += [('a_%i'%j, "%sfold_%i/augment_1_%i/runs/augment/results.csv"%(experiment_dir, 10+i, j), 10+i)]
    ...
    for i in i_:
        # try:
        dfs.append(('raw_%i'%raw_size, pd.read_csv("csv/fold_t_%i.csv"%i).iloc[0:300], i))
        for j in aug_factors:
            dfs.append(('aug_%i'%j, pd.read_csv("csv/fold_a_%i_%i.csv"%(j, i)).iloc[0:300], i))
        # except:
        #     pass
    print(dfs)
    def cs(secs, df):
    ...
    kfold['raw_%i'%raw_size]['raw'] = kfold['raw_%i'%raw_size].mean(axis=1)
    ...
    kfold['raw_%i'%raw_size].plot(ax=ax, x='cumtime', y='raw', linestyle='solid', color='black', title='%i-fold Monte Carlo cross validation mean mAP per epoch'%k)
    plt.fill_between(color='lightgray', x=kfold['raw_%i'%raw_size]['cumtime'], y1=kfold['raw_%i'%raw_size]['raw'] - kfold['raw_%i'%raw_size]['std']/(2*math.sqrt(k)), y2=kfold['raw_%i'%raw_size]['raw'] + kfold['raw_%i'%raw_size]['std']/(2*math.sqrt(k)))
